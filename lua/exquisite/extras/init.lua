...
---@class exquisite.Extra
---@field ext string file extension (empty for none)
---@field render fun(c: exquisite.Colors, ansi: string[], name: string): string

--- tool -> module
M.tools = {
  ghostty = "exquisite.extras.ghostty",
  kitty = "exquisite.extras.kitty",
  lazygit = "exquisite.extras.lazygit",
}

---@param dir? string
---@return string dir
function M.generate(dir)
  -- <root>/lua/exquisite/extras/init.lua -> <root>/extras
  dir = dir or (vim.fn.fnamemodify(debug.getinfo(1, "S").source:sub(2), ":p:h:h:h:h") .. "/extras")
  local ex = require("exquisite")
  local terminal = require("exquisite.terminal")

  for tool, mod in pairs(M.tools) do
    local extra = require(mod) ---@type exquisite.Extra
    local out = vim.fs.joinpath(dir, tool)
    vim.fn.mkdir(out, "p")
    for _, variant in ipairs({ "light", "dark" }) do
      local c = ex.colors(variant)
      local name = "exquisite-" .. variant
