...
---@param hl table<string, exquisite.Style>
...
function M.source(name, variant, hl, terminal)
  ...
  local lines = {
    ...
    -- exquisite variants overwrites everything and `hi clear` is pure waste.
    'local prev=g.colors_name',
    'if prev and prev:sub(1,9)~="exquisite" then vim.cmd("hi clear") end',
...
function M.write(src, path)
  local fn = assert(load(src, "=exquisite"))
