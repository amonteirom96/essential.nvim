...
---@param hl table<string, essential.Style>
...
function M.source(name, variant, hl, terminal)
  ...
  local lines = {
    ...
    -- essential variants overwrites everything and `hi clear` is pure waste.
    'local prev=g.colors_name',
    'if prev and prev:sub(1,9)~="essential" then vim.cmd("hi clear") end',
...
function M.write(src, path)
  local fn = assert(load(src, "=essential"))
