...
local ACCENTS = { "red", "orange", "yellow", "green", "cyan", "azure", "blue", "purple" }
...
local function banner()
  ...
  local function title(c, clip)
    ...
    add(
      '<text x="%d" y="258" text-anchor="middle" font-family="%s" font-size="26" fill="%s" letter-spacing="0.5">monochrome code · color only where it means something</text>',
      W / 2, SANS, c.fg
    )
    add(
...
local MENU = {
  { "hex", "Function", "blue", "ƒ" },
  { "highlights", "Field", "azure", "◆" },
  { "hue", "Variable", "cyan", "x" },
  { "hsl", "Class", "yellow", "C" },
  { "http", "Module", "purple", "M" },
